class App{
    ...
    getForm(){
        const sheet = SpreadsheetApp.getActive().getSheets().find(sheet => sheet.getFormUrl())
        if (!sheet) return
    ...
    createDropdowns(form){
        ...
        const kidPages = []
        const parentDropdown = form.addListItem().setTitle(parentTitle).setRequired(true)
        
        const choices = Object.keys(items).map(parent => {
            const page = form.addPageBreakItem().setTitle(parent)
            form.addListItem().setTitle(childTitle).setChoiceValues(items[parent]).setRequired(true)
    ...
    createForm(){
        ...
        const title = values[0][1] + " - " + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd hh:mm:ss")
        ...
        const destination = DriveApp.getFileById(SpreadsheetApp.getActive().getId()).getParents().next()
        ...
        const sheet = SpreadsheetApp.getActive().getSheetByName(this.sheetName) || SpreadsheetApp.getActive().insertSheet(this.sheetName)
        sheet.clear()
...
function createForm(){
    ...
    try{
        ...
        message = `The new form has been created successfully and linked to sheet "${app.responseSheetName}".`
    } catch(err) {
        message = err.message
        title = "Error"
    }
...
function onOpen(){
    ...
    const trigger = triggers.find(trigger => trigger.getHandlerFunction() === "onFormSubmit")
...
function createTrigger(){
    ...
    ScriptApp.newTrigger(functionName).forSpreadsheet(SpreadsheetApp.getActive()).onFormSubmit().create()
